...
void setup()  //SETUP, Sets pins and stuff
{
  draw_elecrow();
  delay(2000);
  Wire.begin();
...
void loop() // MAIN LOOP
{
  ...
  read_value();                             //get sensor values
  water_flower();
  int button_state = digitalRead(button);
  if (button_state == 1)                 //if button on
  {
    read_value();
    u8g.firstPage();
    do
...
void water_flower()
{
  if (moisture1_value < 30) //low moisture, needs water
  {
    digitalWrite(relay1, HIGH); //set valve1 high (open valve1)
    relay1_state_flag = 1;      //set valve1 state
    delay(50);                  //wait 50s
    if (pump_state_flag == 0)   //if pump not on (could be on from earlier readings)
    {
      digitalWrite(pump, HIGH); //set pump on
      pump_state_flag = 1;      //set pump state on
      delay(50);                //wait 50s
    }
  }
