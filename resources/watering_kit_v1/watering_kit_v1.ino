...
void loop() // MAIN LOOP
{
  ...
  //water_flower();
  delay(1000);
  int button_state = digitalRead(button);   //check button
  if (button_state == 1)                    //if button on (proably just screen, everything still works w/o button)
  {
    read_value();                           //read value
    u8g.firstPage();                        //something with screen setup?
    do
    {
      drawTH();
      drawflower();
...
void drawTH(void)     //probably draws temperatures
{
  int A = 0;
  int B = 0;
  int C = 64;
